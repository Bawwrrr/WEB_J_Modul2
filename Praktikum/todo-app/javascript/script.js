const addActivityFieldInput = document.querySelector(".add-activity-field-input");
const addActivityFieldButton = document.querySelector(".add-activity-field-button");
const editActivityFieldButton = document.querySelector(".edit-activity-field-button");
const listActivitiesItemContainer = document.querySelector(".list-activities-item-container");
...
editActivityFieldButton.addEventListener("click", () => {
    ...
    listActivitiesItemContainer.children[indexOfElement].children[0].innerHTML = addActivityFieldInput.value;
...
listActivitiesItemContainer.addEventListener("click", (event) => {
    ...
    if (event.target.classList.contains("bx-check")) {
        ...
        if (elementToCheck) {
            const pervInnerHtml = elementToCheck.querySelector(".list-activities-item-text").innerHTML;
            elementToCheck.querySelector(".list-activities-item-text").innerHTML = `<s>${pervInnerHtml}<s/>`;
        }
    }
    if (event.target.classList.contains("bx-edit-alt")) {
        ...
        if (elementToEdit) {
            const pervInnerHtml = elementToEdit.querySelector(".list-activities-item-text").innerHTML;
            addActivityFieldInput.value = pervInnerHtml;
            const index = Array.from(listActivitiesItemContainer.children).indexOf(elementToEdit);
